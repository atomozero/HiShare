...
// MUSCLE 6.11 auto-selects PODHashFunctor for POD key types like node_ref (via
// AutoChooseHashFunctorHelper), so the old explicit HashFunctor<node_ref>
// specialization is no longer needed -- the default POD hashing handles it.
...
public:
    ...
    */
   void SendConnectBackRequestMessage(const char * targetSessionID, uint16 port, bool useSSL);

   /* Sets whether we require TLS-encrypted transfers.  This is advertised to peers as the
    * "supports_ssl" capability and re-publishes our name node so peers see the change. */
   void SetRequireTLS(bool requireTLS);
   bool GetRequireTLS() const {return _requireTLS;}
...
private:
   ...
   bool _firewalled;
   bool _requireTLS;   // true iff we advertise "supports_ssl" and require TLS on our incoming transfers
