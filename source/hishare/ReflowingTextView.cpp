...
void ReflowingTextView :: UpdateToolTip()
{
   if ((_canShow)&&(_currentLinkStart >= 0))
   {
      if (_urlTip == NULL) 
      {
         ...
         // Use the system tool-tip colours so the URL popup matches the theme.
         const rgb_color tipBg     = ui_color(B_TOOL_TIP_BACKGROUND_COLOR);
         const rgb_color tipText   = ui_color(B_TOOL_TIP_TEXT_COLOR);

         BView * tipBorder = new BView(_urlTip->Bounds(), NULL, B_FOLLOW_ALL_SIDES, 0);
         tipBorder->SetViewColor(tipText);
         _urlTip->AddChild(tipBorder);

         BRect inset = tipBorder->Bounds();  inset.InsetBy(1,1);
         BView * tipInterior = new BView(inset, NULL, B_FOLLOW_ALL_SIDES, 0);
         tipInterior->SetLowColor(tipBg);
         tipInterior->SetViewColor(tipBg);
         tipBorder->AddChild(tipInterior);

         _urlStringView = new BStringView(tipInterior->Bounds(), NULL, "", B_FOLLOW_ALL_SIDES);
         _urlStringView->SetLowColor(tipBg);
         _urlStringView->SetViewColor(tipBg);
         _urlStringView->SetHighColor(tipText);
         tipInterior->AddChild(_urlStringView);
      }
      if (_urlTip->Lock())
      {
