...
// Haiku Locale Kit bridge context: the catkeys extracted for HiShare live under this
// context (see the generated .catkeys).  Translations are keyed by the English string.
...
int GetDefaultLanguageForLocale()
{
   ...
   // Prefer the Haiku system language (Locale preferences) so HiShare follows the system
   // language like a native Haiku app -- this is what drives the built-in string tables
   // now that the per-app Language menu is gone (the Locale Kit catalog, when installed,
   // follows the same system language).  Fall back to the POSIX env vars, then English.
   {
      BMessage prefLangs;
      const char * code;
      if ((BLocaleRoster::Default()->GetPreferredLanguages(&prefLangs) == B_OK)
        &&(prefLangs.FindString("language", 0, &code) == B_OK))
      {
         String sysLang(code);
         sysLang = sysLang.Substring(0, "_");  // "it_IT" / "en_150" -> "it" / "en"
         for (uint32 i=0; i<ARRAYITEMS(posixLocaleMapping); i++) if (sysLang == posixLocaleMapping[i].posix) return posixLocaleMapping[i].language;
      }
   }
...
static const char * _englishStrings[NUM_STRINGS] = {
   ...
   "Error, couldn't create connect thread! (Try quitting HiShare, then Restarting Networking)",
   "(Sharing %i files)",
   ...
   "Name",
   "ID",
   "HiShare is listening on port %u.  Select Connect from the File menu to begin!",
   "Couldn't start file-sharing server thread!  (Try Restarting Networking, then restarting HiShare)",
   ...
   "Changed Query to",
   "Started Query",
   "Couldn't start ShareFileTransfer session to serve remote transfer request!  (Try Restarting Networking, then restarting HiShare)",
   ...
   "English",
   "Spanish",
   "Language %s selected.  Change will take effect after you restart HiShare.",
   "Language",
   "German",
   "Kind",
   "File Size",
   ...
   "Added server: %s",
   "Removed server: %s",
   "HiShare v%s is now available for download.  You can upgrade at %s",
...
static const char * _spanishStrings[NUM_STRINGS] = {
   ...
   "Nombre",
   "ID",
   "HiShare está escuchando en el puerto %u.  ¡Seleccionar Conectar en el menú \"Archivo\" para comenzar!",
   ...
   "Inglés",
   "Español",
   "Idioma %s seleccionado. El cambio tendrá efecto cuando reinicies HiShare",
   "Idioma",
   "Alemán",
   "Tipo",
   "Tamaño",
   ...
   "HiShare Ventana Privada de Chat",
   "Chatear Con:",
   ...
   "Servidor añadido: %s",
   "Servidor quitado: %s",
   "La version v%s de HiShare está disponible para descargar. Puede actualizarla a %s",
...
static const char * _germanStrings[NUM_STRINGS] = {
   ...
   "Fehler: konnte keinen Verbindungs-Thread erstellen! (Versuchen Sie, HiShare zu beenden und danach das Netzwerk neuzustarten)",
   ...
   "Name",
   "ID",
   "HiShare lauscht auf Port %u.  Wählen Sie Verbinden im Menu Datei, um loszulegen!",
   ...
   "Englisch",
   "Spanisch",
   "Sprache %s ausgewählt.  Sie müssen HiShare neu starten, damit die Änderungen wirksam werden.",
   "Sprache",
   "Deutsch",
   "Dateiart",
   "Dateigröße",
   ...
   "Aus",
   "Es sind noch %i Übertragungen im Gang. Sind Sie sicher, dass Sie HiShare beenden wollen?",
   "Nicht beenden",
   "Änderungsdatum",
   "Esperanto",
   "Schwedisch",
   ...
   "Server %s entfernt",
   "HiShare v%s ist jetzt erhältlich zum Download. Upgraden Sie auf %s",
...
static const char * _dutchStrings[NUM_STRINGS] = {
   ...
   "Naam",
   "Nr",
   "HiShare luistert op poort %i. Selecteer verbinden in het bestands menu om te beginnen!",
   ...
   "Engels",
   "Spaans",
   "Taal %s geselekteerd. De veranderingen vinden plaats na het herstarten van HiShare.",
   "Taal",
   "Duits",
   "Soort",
   "Bestands Grootte",
   ...
   "HiShare v%s is nu verkrijgbaar, u kunt opwaarderen op %s",
...
static const char * _portugueseStrings[NUM_STRINGS] = {
   ...
   "Nome",
   "ID",
   "HiShare está escutando na porta %u.  Selecione Conectar no menu Arquivo para iniciar!",
   ...
   "Inglês",
   "Espanhol",
   "Lingua %s selecionada.  A mudança se tornará efetiva após você reiniciar o HiShare.",
   "Lingua",
   "Alemão",
   "Tipo",
   "Tamanho do Arquivo",
   ...
   "Janela de Conversação Privada do HiShare",
   "Conversar com:",
   ...
   "Removido servidor: %s",
   "HiShare v%s está disponivel para download. Você pode atualizar em %s",
...
static const char * _frenchStrings[NUM_STRINGS] = {
   ...
   "Nom",
   "N°",
   "HiShare utilise le port %u.  Sélectionnez Connecter du menu Fichier pour commencer!",
   ...
   "Anglais",
   "Espagnol",
   "Langue %s sélectionnée.  Le changement prendra effet après avoir redémarré HiShare.",
   "Langue",
   "Allemand",
   "Type",
   "Taille du Fichier",
   ...
   "Fenêtre Privée de Discussion de HiShare",
   "Discussion avec:",
   ...
   "Serveur ajouté: %s",
   "Serveur supprimé: %s",
   "HiShare v%s est maintenant disponible.  Vous pouvez le télécharger sur %s",
...
static const char * _italianStrings[NUM_STRINGS] = {
   ...
   "Errore, impossibile creare thread di connessione! (Prova ad uscire da HiShare, e poi a far ripartire la rete)",
   ...
   "Nome",
   "ID",
   "HiShare sta ascoltando sulla porta %u.  Scegli Collegati dal menu File per iniziare!",
   "Impossibile iniziare thread di condivisione server!  (Prova a far ripartire la rete, poi a rilanciare HiShare)",
   ...
   "Ricerca cambiata in",
   "Ricerca attivata",
   "Impossibile iniziare sessione ShareFileTransfer per rispondere a richiesta di trasferimento remoto!  (Prova a far ripartire la rete, poi a rilanciare HiShare)",
   ...
   "Inglese",
   "Spagnolo",
   "Lingua %s selezionata.  La modifica avrà effetto quando rilancerai HiShare.",
   "Lingua",
   "Tedesco",
   "Tipo",
   "Dimensione File",
   ...
   "HiShare Finestra Privata di Chat",
   "Chatta Con:",
...
static const char * _russianStrings[NUM_STRINGS] = {
   ...
   "Ошибка, не удается присоединиться! (Закройте HiShare и нажмите Restart Networkiing в настройках сети)",
   ...
   "Имя",
   "№",
   "HiShare следит за портом %i.  Выберите Присоединиться в меню Файл!",
   "Couldn't start file-sharing server thread!  (Try Restarting Networking, then restarting HiShare)",
   ...
   "Английский",
   "Испанский",
   "Выбран %s язык.  Перезагрузите HiShare, чтобы изменения вступили в силу.",
   "Язык интерфейса",
   "Немецкий",
   "Тип",
   "Размер",
...
static const char * _esperantoStrings[NUM_STRINGS] = {
   ...
   "Eraro, ne eblis krei konektan fadenon! (Provu eliru HiShare-on, poste Restartigu la Retumadon)",
   ...
   "Nomo",
   "ID",
   "HiShare aŭskultas ĉe la pordo %u.  Elektu Konektu en la Arkivo-menuo por komenci!",
   "Ne eblis startigi la dosier-disponigan fadenon!  (Provu Restartigu la Retumadon, poste restartigu HiShare-on)",
   ...
   "Ŝanĝis Serĉo al",
   "Komencis Serĉi",
   "Ne eblis startigi ShareFileTransfer-sesion por servi la foran transigpeton!  (Try Restarting Networking, then restarting HiShare)",
   ...
   "Angla",
   "Hispana",
   "La lingvo %s estis elektita.  La ŝanĝo efikos post restartigo de HiShare.",
   "Lingvo",
   "Germana",
   "Tipo",
   "Grandeco",
   ...
   "Aldonis la servilon",
   "Forigis la servilon",
   "HiShare v%s nun elŝuteblas. Vi povas ĝisdatigi ĉe %s",
   "execute shell command",
   "Executing shell command",
   "shell command",
   "idle command set to",
...
static const char * _swedishStrings[NUM_STRINGS] = {
   ...
   "Fel, kunde inte skapa anslutningstråd! (Försök med att avsluta HiShare, och Starta om Nätverket)",
   ...
   "Namn",
   "ID",
   "HiShare lyssnar på port %u. Välj Anslut i Arkiv-menyn för att börja!",
   "Kunde inte starta servertråden för fildelning! (Försök med att Starta om Nätverket, och sedan starta om HiShare)",
   ...
   "Startade Sökning",
   "Kunde inte starta ShareFileTransfer-session för att betjäna andra sidans överföringsbegäran!  (Försök med att Starta om Nätverket, och sedan starta om HiShare)",
   ...
   "Engelska",
   "Spanska",
   "Språk %s valt.  Ändringarna kommer att ta effekt efter att du startat om HiShare.",
   "Språk",
   "Tyska",
   "Typ",
   "Filstorlek",
   ...
   "Lade till server",
   "Tog bort server",
   "HiShare v%s finns nu för nerladdning. Du kan uppgradera på %s",
...
static const char * _norwegianStrings[NUM_STRINGS] = {
   ...
   "Feil, kan ikkje laga tilkoplingstråd! (Prøv å avslutta HiShare og starta Netverkstenester på nytt)",
   ...
   "Namn",
   "Nummer",
   "HiShare lyttar på port %u.  Vel Kopla til frå Fil-menyen for å byrja!",
   "Kan ikkje starta tenartråd for fildeling!  (Prøv å starta Netverkstenester på nytt, så HiShare)",
   ...
   "Starta førespurnad",
   "Kan ikkje starta ShareFileTransfer-omgang for å svara på førespurnad frå ein annan brukar!  (Prøv å starta Netverkstenester på nytt, så HiShare)",
   ...
   "Engelsk",
   "Spansk",
   "Språket %s er valt.  Endringane vil synast når du startar HiShare på nytt att.",
   "Språk",
   "Tysk",
   "Type",
   "Storleik",
   ...
   "Har fjerna tenar: %s",
   "No kan du få HiShare v%s. Du kan laste han ned frå %s.",
...
static const char * _serbianStrings[NUM_STRINGS] = {
   ...
   "Greska, nije se mogla uspostaviti konekcija! (Ugasite HiShare i restartujte Networking)",
   ...
   "Ime",
   "ID",
   "HiShare slusa na portu %u.  Izaberi Connect iz Datoteka menija da pocnes!",
   "Nije se mogao pokrenuti ile-sharing server thread!  (Restartujte Networking i pokrenite HiShare!)",
   ...
   "Promenjen upit u",
   "Pokrenut upit",
   "Nije pokrenuta ShareFileTransfer sesija da bi se zadovoljio zahtev! (Restartuj Networking,a potom restartuj HiShare)",
   ...
   "Engleski",
   "Spanski",
   "Jezik %s izabran. Promena ce se prikazati po ponovnom pokretanju HiShare-a.",
   "Jezik",
   "Nemacki",
   "Tip",
   "Velicina datoteke",
   ...
   "HiShare prozor za privatni razgovor",
   "razgovor sa:",
...
static const char * _bulgarianStrings[NUM_STRINGS] = {
   ...
   "Грешка, не може да се създаде нишка! (Затворете HiShare и натиснете Restart Networkiing в мрежовите настройки)",
   ...
   "Име",
   "№",
   "HiShare следи порт %i.  Изберете Connect в менюто Файл!",
   "Не мога да стартирам file-sharing server thread!  (Опитай Restarting Networking, след това стартирай HiShare)", //*
   ...
   "Английски",
   "Испански",
   "Избран е %s език.  Промяната ще настъпи след рестартиране на HiShare.",
   "Език",
   "Немски",
   "Значение",
   "Размер",
   ...
   "Добавен е сървър: %s",
   "Изтрит е сървър: %s",
   "HiShare v%s е достъпна за сваляне.  Можете да свалите версия %s",
   "execute shell command",
   "Executing shell command",
   "shell command",
   "idle command set to",
...
static const char * _koreanStrings[NUM_STRINGS] = {
   ...
   "에러, 연결 쓰레드를 생성할 수 없음! (HiShare를 종료하시오, 그런 후 네트워킹을 재시작하시오)",
   "(당신 컴퓨터의 %i 개 파일을 공유중)",
   ...
   "이름",
   "ID",
   "HiShare is listening on port %u.  시작하기 위해 파일메뉴의 접속할서버를 누르시오!",
   "Couldn't start file-sharing server thread!  (네트워킹을 재시작한 후 HiShare를 다시 실행하시오)",
   "사용자로 부터 내려받을 수 없음. ",
   ", 이용가능한 접속 정보가 없음.",
   "Can't download files from ",
   ", because both of us are behind firewalls.",
   ...
   "Changed Query to",
   "Started Query",
   "Couldn't start ShareFileTransfer session to serve remote transfer request!  (네트워킹을 재시작한 후 HiShare를 다시 실행하시오)",
   "변경된 당신의 이름은 ",
   "by",
   "알수없는 사용자 ",
   ", 메세지 \"",
   ...
   "영어",
   "스페인어",
   "언어가 %s 로 선택됨.  변경사항은 HiShare를 재 실행하셔야 적용됩니다.",
   "언어",
   "독일",
   "유형",
   "파일 크기",
...
static const char * _hungarianStrings[NUM_STRINGS] = {
   ...
   "Hiba, nem lehet kapcsolódni. Próbálkozz azzal, hogy kilépsz a HiShare programból és újraindítod a hálózatot",
   ...
   "Név",
   "ID",
   "HiShare kilistázza a  %u portot. A továbbiakban (ha nincs beállítva az azonnali csatlakozás) válaszd a Fájl menüben a 'Csatlakozás' parancsot!",
   "Nem lehet indítani a fájlmegosztást! (Próbálkozz azzal, hogy kilépsz a HiShare programból és újraindítod a hálózatot)",
   ...
   "Keresés indítása",
   "Nem lehet elindítani a megosztott fájlok szakaszainak átvitelét a távoli géprõl. (Próbálkozz azzal, hogy kilépsz a HiShare programból és újraindítod a hálózatot)",
   ...
   "Angol",
   "Spanyol",
   "A %s nyelv lett kiválasztva. A változásokhoz újra kell indítani a HiShare-t.",
   "Nyelv",
   "Német",
   "Típus",
   "Fájl méret",
   ...
   "HiShare v%s most elérhetõ a letöltéshez. Innen frissítheted: %s",
...
//------------------------------------------------------------------------------------
// Romanian language update for HiShare... wohooo!
// Ati vrut HiShare in romana, asa ca am stat vreo doua ore sa traduc
...
static const char * _romanianStrings[NUM_STRINGS] = {
   ...
   "Eroare, nu s-a putut crea thread conexiune! (Incearca sa iesi din HiShare, dupa care Reporneste Networking)",
   ...
   "Nume",
   "ID",
   "HiShare asculta pe portul %u.  Selecteaza Conectare de la meniul Fisier ca sa incepi!",
   "Nu am putut porni thread-ul de server file-sharing!  (Incearca sa Repornesti Networking si apoi HiShare)",
   ...
   "Schimbat Cautare in",
   "Cautare Pornita",
   "Nu s-a putut porni sesiunea de ShareFileTransfer pentru a servi transferului cerut!  (Incearca sa Repornesti Networking, dupa care lanseaza HiShare)",
   ...
   "Engleza",
   "Spaniola",
   "Limba %s selectata.  Schimbarile vor avea efect la repornirea HiShare.",
   "Limba",
   "Germana",
   "Tip",
   "Marime Fisier",
   ...
   "Fereastra Privata de Chat HiShare",
   "Vorbeste cu:",
   ...
   "Adaugat server: %s",
   "Sters server: %s",
   "HiShare v%s este disponibil pentru download.  Poti face un upgrade la %s",
...
static const char * _schineseStrings[NUM_STRINGS] = {
   ...
   "错误！开始下载于延时连接！",
   "错误！开始下载于延时连接 — 无收发线程！",
   "错误！无法创建连接线程（请尝试关闭 HiShare，并重新启动网络服务）！",
   "（已共享出 %i 个本地文件）",
   ...
   "名称",
   "序列号",
   "HiShare 正在监停 %u 端口。 请点击“文件”菜单中的“连接”开始。",
   "无法开启共享服务器线程！ （请重新启动网络服务并重新启动 HiShare）",
   "无法下载于 ",
   ", 无可用连接信息.",
   "无法下载文件从 ",
   "， 因为我们两个都在防火墙后面。",
   ...
   "改变查询参数为",
   "开始查询",
   "无法发送文件共享传输对话请求到远程服务器！（请尝试在重新启动网络服务后重新启动 HiShare）",
   "你的昵称已改变为： ",
   "by",
   "未知用户 ",
   ", 消息 \"",
...
static const char * _turkishStrings[NUM_STRINGS] = {
   ...
   "Bağlantı tret'i kurulamadı! (HiShare'i kapatınız, 'Networking'i tekrar başlatınız)",
   ...
   "İsim",
   "ID",
   "HiShare port %u üzerinden dinliyor.  Başlamak için lütfen Yansıya bağlanın!",
   "Dosya Paylaşım sunucu tred'ini başlatamadım!  (Networking'i tekrar başlat, ondan sonra HiShare'i tekrar başlat)",
   ...
   "Sorgulama değişmesi",
   "Arama başlatıldı",
   "Karşıdan gelen transfer isteğine Dosya Paylaşım Transferi başlatılamadı!  (Networking'i tekrar başlat, ondan sonra HiShare'i tekrar başlat)",
   ...
   "İngilizce",
   "İspanyolca",
   "%s dili seçildi.  Bir dahaki HiShare açılışında etkinleşecek.",
   "Dil",
   "Alman",
   "Nev'i",
   "Dosya büyüklüğü",
   ...
   "Italyanca",
   "HiShare başlangıcında otomatik olarak yansıya bağlan",
   "Özel mesajlar",
   "Özel mesaj penceresini aç",
   "HiShare özel mesaj penceresi",
   "Çet yapılacak kişi:",
   "Kullanıcı seçilmedi!",
   "Ayarları hatırla",
   ...
   "Kapat",
   "Dikkat! %i transfer devam etmekte, HiShare'i kapatmaya emin misin?",
   "Hayır kapatma",
   "Modifiye zamanı",
   "Esperanto",
   "İsveç'ce",
   ...
   "Eklenilen Yansı: %s",
   "Silien Yansı: %s",
   "HiShare v%s mevcut! Güncelleme için %s 'den indirmeniz mümkün.",
   "komut gir (şel)",
   "komut uygulanıyor",
   "şel komutu",
   "boş komutu ayarla",
...
static const char * _japaneseStrings[NUM_STRINGS] = {
   ...
   "エラー, 接続スレッドを作成できません! (HiShare を終了し、Network の再起動を試みてください)",
   "(共有 %i ローカルファイル)",
   ...
   "ユーザ名",
   "ID",
   "HiShare はポート %u で待ち受け中です。ファイルメニューの\"接続 :\"を選択し開始してください。",
   "ファイル共有 サーバ スレッドが開始できません!  (Network の再起動を試みてください。その後 HiShare を開始してください。)",
   ...
   "変更するクエリは",
   "クエリの開始",
   "リモートサーバへ転送を要求しましたが、ファイル共有転送セッションが開始できません!  (Network の再起動を試みてください。その後 HiShare を開始してください。)",
   ...
   "English",      //英語
   "Spanish",      //スペイン語
   "言語に %s が選択されました。変更内容は HiShare の再起動後に適用されます。",
   "言語設定(A)",
   "German",      //ドイツ語
   "種類",
   "サイズ",
   ...
   "サーバを追加: %s",
   "サーバを除去: %s",
   "最新版の HiShare v%s がダウンロード可能です。 %s にてアップグレードしてください。",
   "シェルコマンドの実行",
   "シェルコマンドの実行中",
   "シェルコマンド",
   "待機コマンドの設定",
...
static const char * _finnishStrings[NUM_STRINGS] = {
   ...
   "Virhe, säiettä ei voitu yhdistää! (Kokeile sammuttaa HiShare ja käynnistää Networking uudelleen)",
   "(Jaossa %i tiedostoa)",
   ...
   "Nimi",
   "ID",
   "HiShare kuuntelee porttia %u.  Valitse Yhdistä Tiedosto-valikosta aloittaaksesi!",
   "Tiedostonjaon palvelinsäiettä ei voitu käynnistää!  (Yritä käynnistää Networking uudelleen ja sitten uudelleenkäynnistää HiShare)",
   ...
   "Haku Muutettu",
   "Haku Aloitettu",
   "ShareFileTransfer-sessiopalvelua ei voitu aloittaa etäsiirtopyyntöä varten!  (Yritä käynnistää Networking uudelleen ja sitten uudelleenkäynnistää HiShare)",
   ...
   "Englanti",
   "Espanja",
   "Kieleksi on valittu %s.  Muutos tapahtuu kun käynnistät HiSharen uudelleen.",
   "Kieli",
   "Saksa",
   "Tyyppi",
   "Tiedoston Koko",
   ...
   "HiSharen Yksityisviestit Ikkuna",
   ...
   "HiShare v%s on nyt ladattavissa.  Voit päivittää osoitteessa %s",
...
const char * str(int ws, int language)
{
   ...
   const char * builtin = ((base)&&(ws >= 0)&&(ws < NUM_STRINGS)) ? base[ws] : NULL;

   // Haiku Locale Kit bridge: when the caller isn't forcing a specific language and this is a
   // normal UI string (not a \001-prefixed attribute-column key), prefer a translation from the
   // system message catalog (BCatalog), keyed by the English text.  If no catalog / no matching
   // entry is present, GetString() returns the key unchanged and we fall back to the built-in
   // tables -- so behaviour is identical to before when no catkeys are installed.  This lets the
   // app follow the Haiku system language and lets translators contribute standard .catkeys.
   if ((language < 0)&&(ws >= 0)&&(ws < NUM_STRINGS))
   {
      const char * eng = _englishStrings[ws];
      if ((eng)&&(eng[0] != '\001'))
      {
         const char * tr = BLocaleRoster::Default()->GetCatalog()->GetString(eng, B_TRANSLATION_CONTEXT);
