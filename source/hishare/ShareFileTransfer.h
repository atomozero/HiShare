...
{ 
public:
   OffsetAndPath() : _offset(0), _maxBytes(-1) {/* empty */}
   OffsetAndPath(off_t offset, const String & path, off_t maxBytes = -1) : _offset(offset), _path(path), _maxBytes(maxBytes) {/* empty */}

   void SaveToArchive(BMessage & archive) const
   {
      archive.AddString("path", _path());
      archive.AddInt64("offset", _offset);
      archive.AddInt64("maxbytes", _maxBytes);
   }
   ...
   void SetFromArchive(const BMessage & archive)
   {
      ...
      if (archive.FindInt64("maxbytes", &_maxBytes) != B_NO_ERROR) _maxBytes = -1;
   }

   off_t _offset;
   String _path;
   off_t _maxBytes;   // max bytes to transfer from _offset for this file (-1 = to end-of-file); used for resume/swarming byte-range requests
};
...
public:
    ...
    */
   void AddRequestedFileName(const char * fileName, off_t startByteOffset, const char * path, BPoint *point, off_t maxBytes = -1);
   ...
   /* Enables TLS encryption for this transfer's connection.  Must be called BEFORE the
    * Init*Session() method.  The TCP-accepting side (InitAcceptSession/InitSocketUploadSession)
    * acts as the TLS server; the TCP-connecting side (InitConnectSession) as the TLS client.
    * Both peers must agree (negotiated via the "supports_ssl"/"use_ssl" flags) or the handshake fails. */
   void SetUseTLS(bool useTLS) {_useTLS = useTLS;}
   bool GetUseTLS() const {return _useTLS;}
    ...
    * once in accept/connect-back mode.  One-shot, to prevent retry cycles. */
   bool TriedConnectBackFallback() const {return _triedConnectBackFallback;}

   /* Forgets the remote host's direct address, so that a subsequent
    * RestartSession() re-initializes us in accept (connect-back) mode
    * instead of connecting out to the same unreachable address again. */
   void ForgetRemoteAddress() {_remoteHostName = ""; _remotePort = 0;}
...
private:
   ...
   ThreadWorkerSessionRef GetTransferSessionRef(bool sslServer = false);
   ...
   off_t _currentFileStartOffset;   // byte offset this file's upload started at (for byte-range bounding)
   off_t _currentFileMaxBytes;      // max bytes to send from _currentFileStartOffset (-1 = to EOF)
   ...
   bool _useTLS;             // true iff this transfer's connection should be TLS-encrypted
   ...
   bool _triedConnectBackFallback;  // true iff we already retried this download via connect-back
