...
ShareSettingsWindow::_MakeTransfersCard(const BMessage & s)
{
	...
	BLayoutBuilder::Group<>(v, B_VERTICAL, B_USE_SMALL_SPACING)
		...
		.AddStrut(6)
		.Add(NewCheck(str(STR_ST_AUTO_SPEED), ShareWindow::SHAREWINDOW_COMMAND_TOGGLE_AUTO_DETECT_SPEED, s.GetBool("autodetectspeed"), _target))
...
ShareSettingsWindow::_MakeChatCard(const BMessage & s)
{
	...
	BLayoutBuilder::Group<>(v, B_VERTICAL, B_USE_SMALL_SPACING)
		...
		.Add(NewCheck(str(STR_ST_HIDE_OWN_ECHO), ShareWindow::SHAREWINDOW_COMMAND_TOGGLE_HIDE_OWN_ECHO, s.GetBool("hideownecho", true), _target))
		.AddStrut(4)
