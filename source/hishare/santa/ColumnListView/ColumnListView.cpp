...
fDownArrow(BRect(0.0,0.0,10.0,10.0),B_COLOR_8_BIT,CLVDownArrowData,false,false)
{
	...
	fBgColor = ui_color(B_DOCUMENT_BACKGROUND_COLOR);
	...
	*ContainerView = fScrollView;
	...
	fSelectedItemColorWindowActive = ui_color(B_LIST_SELECTED_BACKGROUND_COLOR);
	fSelectedItemColorWindowInactive = ui_color(B_LIST_SELECTED_BACKGROUND_COLOR);
	fWindowActive = false;
}
...
rgb_color ColumnListView::RowColor(int32 index) const
{
	// Even rows: the plain background.  Odd rows: a subtle stripe made by blending
	// the background a few percent toward the text colour, which darkens it on a
	// light theme and lightens it on a dark theme (so it works either way).
	if ((index & 1) == 0)
		return fBgColor;
	const float t = 0.11f;
	rgb_color c = fBgColor;
	c.red   = (uint8)(c.red   + (fTextColor.red   - c.red)   * t);
	c.green = (uint8)(c.green + (fTextColor.green - c.green) * t);
	c.blue  = (uint8)(c.blue  + (fTextColor.blue  - c.blue)  * t);
	return c;
}
