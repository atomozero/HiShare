...
void CLVEasyItem::SetColumnContent(int column_index, const char *text, bool truncate, bool right_justify)
{
	...
	{
		((int64*)m_column_types.Items())[column_index] = CLVColNone;
	...
	else
	{
		...
		if(!truncate)
		{
			((int64*)m_column_types.Items())[column_index] = CLVColStaticText;
			((char**)m_aux_content.Items())[column_index] = NULL;
		}
		else
		{
			((int64*)m_column_types.Items())[column_index] = CLVColTruncateText|CLVColFlagNeedsTruncation;
			copy = new char[strlen(text)+3];
			strcpy(copy,text);
			((char**)m_aux_content.Items())[column_index] = copy;
		}
		if(right_justify)
			((int64*)m_column_types.Items())[column_index] |= CLVColFlagRightJustify;
	}
}
	...
	bool right_justify)
{
	...
	{
		if(copy)
			((int64*)m_column_types.Items())[column_index] = CLVColBitmap|CLVColFlagBitmapIsCopy;
		else
			((int64*)m_column_types.Items())[column_index] = CLVColBitmap;
		if(right_justify)
			((int64*)m_column_types.Items())[column_index] |= CLVColFlagRightJustify;
		BBitmap* the_bitmap;
		if(copy)
		...
		((int64*)m_aux_content.Items())[column_index] = (int32)horizontal_offset;
	}
}
...
void CLVEasyItem::SetColumnUserTextContent(int column_index, bool truncate, bool right_justify)
{
	PrepListsForSet(column_index);
	if(truncate)
		((int64*)m_column_types.Items())[column_index] = CLVColTruncateUserText;
	else
		((int64*)m_column_types.Items())[column_index] = CLVColUserText;
	if(right_justify)
		((int64*)m_column_types.Items())[column_index] |= CLVColFlagRightJustify;
}
...
void CLVEasyItem::DrawItemColumn(BView *owner, BRect item_column_rect, int32 column_index, bool complete)
{
	...
	// Zebra parity from the item's on-screen row (top / row-height), robust for all lists.
	int32 rowIndex = (item_column_rect.Height() > 0.0f) ? (int32)((item_column_rect.top / item_column_rect.Height()) + 0.5f) : 0;
	if(selected)
		color = ((ColumnListView*)owner)->ItemSelectColor();
	else
		color = ((ColumnListView *)owner)->RowColor(rowIndex);
	owner->SetLowColor(color);
	owner->SetDrawingMode(B_OP_COPY);
	// Always paint the row background so the zebra-stripe tint shows even on
	// partial (non-"complete") redraws.
	(void) complete;
		...
		type == CLVColUserText)
	{
		...
		if(type == CLVColTruncateText)
		{
			if(needs_truncation)
			{
				...
				((int64*)m_column_types.Items())[column_index] &= (CLVColFlagNeedsTruncation^0xFFFFFFFF);
			}
...
void CLVEasyItem::ColumnWidthChanged(int32 column_index, float column_width, ColumnListView *the_view)
{
	...
	if(type == CLVColTruncateText)
	{
		...
		if(cached_rect->top <= bounds.bottom && cached_rect->bottom >= bounds.top)
		{
			...
			((int64*)m_column_types.Items())[column_index] |= CLVColFlagNeedsTruncation;
	}
