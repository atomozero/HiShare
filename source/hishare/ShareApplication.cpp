...
#define BESHARE_SETTINGS_FILE_NAME "hishare_settings"
#define BESHARE_USER_KEY_FILE_NAME "hishare_user_key"
...
{
   // Test/scripting hooks (auto-open windows, team-id file, action-forward message,
   // download-all) are only live when this env var is set, so release builds never
   // expose them to other processes.  BESHARE_HOME (portable data dir) is a real
   // feature and is intentionally NOT gated by this.
   _testHooks = (getenv("HISHARE_TEST_HOOKS") != NULL);
...
void ShareApplication :: ReadyToRun()
{
   ...
   if (_testHooks)
   {
      // Test hook: auto-open the auxiliary windows so scripts can screenshot them.
      if (getenv("BESHARE_TEST_OPENWINDOWS"))
      {
         _window->PostMessage(ShareWindow::SHAREWINDOW_COMMAND_OPEN_SETTINGS);
         _window->PostMessage(ShareWindow::SHAREWINDOW_COMMAND_SHOW_COLOR_PICKER);
      }
      if (getenv("BESHARE_TEST_ABOUT")) _window->PostMessage(ShareWindow::SHAREWINDOW_COMMAND_ABOUT);
      if (getenv("BESHARE_TEST_CRACKTRO")) (new PirateDemoWindow())->Show();

      // Test hook: write our own team id so scripts can message us reliably
      // (the shell's $! is not the app's team when launched via nohup).
      const char * teamFile = getenv("BESHARE_TEAM_FILE");
      if (teamFile)
      {
...
void ShareApplication:: MessageReceived(BMessage * msg)
{
   switch(msg->what)
   {
      ...
      case 'FWDW':  // test hook: forward a toolbar action to the window by index
      {
         int32 which;
         if (_testHooks&&(_window)&&(msg->FindInt32("which", &which) == B_NO_ERROR))
         {
            uint32 cmd = ShareWindow::SHAREWINDOW_COMMAND_OPEN_SETTINGS;
            if      (which == 1) cmd = ShareWindow::SHAREWINDOW_COMMAND_SHOW_COLOR_PICKER;
            else if (which == 2) cmd = ShareWindow::SHAREWINDOW_COMMAND_DISCONNECT_FROM_SERVER;
            else if (which == 3) cmd = ShareWindow::SHAREWINDOW_COMMAND_RECONNECT_TO_SERVER;
            _window->PostMessage(cmd);
         }
      }
      break;
      ...
      case 'dnla':  // test hook: download every current query result
         if (_testHooks&&(_window)&&(_window->Lock()))
         {
            _window->DownloadAllResults();
            _window->Unlock();
         }
      break;
