...
#ifdef MUSCLE_AVOID_IPV6
   // NOTE: gethostbyname() returns a shared static hostent and is NOT thread-safe -- two
   // threads resolving DNS at once (e.g. a connect thread and BeShare's PortMapper
   // reachability probe) race on that static buffer and segfault in the ntohl() below.
   // getaddrinfo() allocates a per-call result, so it's safe to call concurrently.
   {
      ...
      {
         for (struct addrinfo * n = result; n != NULL; n = n->ai_next) if (n->ai_family == AF_INET) { ret = ntohl(((struct sockaddr_in *)n->ai_addr)->sin_addr.s_addr); break; }
         freeaddrinfo(result);
      }
   }
#else
