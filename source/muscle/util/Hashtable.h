...
public:
   ...
   // -----------------------------------------------------------------------
   // BeShare / MUSCLE-3.x backwards-compatibility API.  These keep the old
   // dual-cursor semantics (independent key and value iteration) so existing
   // "while((k=GetNextKey())&&(v=GetNextValue()))" style loops keep working
   // unchanged against MUSCLE 6.x.  They use their own cursors and do NOT
   // affect HasData()/GetKey()/GetValue()/operator++.
   const KeyType * GetNextKey()
   {
      if (_bsKeyCookie == NULL) return NULL;
      const KeyType * ret = &_owner->GetKeyFromCookie(_bsKeyCookie);
      _bsKeyCookie = _owner->GetSubsequentEntry(_bsKeyCookie, _flags);
      return ret;
   }
   status_t GetNextKey(KeyType & setNextKey) { const KeyType * k = GetNextKey(); if (k) {setNextKey = *k; return B_NO_ERROR;} return B_ERROR; }

   ValueType * GetNextValue()
   {
      if (_bsValueCookie == NULL) return NULL;
      ValueType * ret = &_owner->GetValueFromCookie(_bsValueCookie);
      _bsValueCookie = _owner->GetSubsequentEntry(_bsValueCookie, _flags);
      return ret;
   }
   status_t GetNextValue(ValueType & setNextValue) { ValueType * v = GetNextValue(); if (v) {setNextValue = *v; return B_NO_ERROR;} return B_ERROR; }

   bool HasMoreKeys()   const { return (_bsKeyCookie   != NULL); }
   bool HasMoreValues() const { return (_bsValueCookie != NULL); }
...
private:
   ...
   // BeShare / MUSCLE-3.x compatibility: independent key- and value-iteration
   // cursors, so the legacy GetNextKey()/GetNextValue() methods (below) keep
   // their old dual-cursor semantics.  Initialised to _iterCookie by every ctor.
   void * _bsKeyCookie;
   void * _bsValueCookie;
...
{
   _bsKeyCookie = _bsValueCookie = _iterCookie;
}
