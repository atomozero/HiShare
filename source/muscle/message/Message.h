...
public:
   ...
   // BeShare / MUSCLE-3.x compatibility: single-cursor "advance and return"
   // helpers, so old "while(iter.GetNextFieldName(x) == B_NO_ERROR)" loops keep
   // working unchanged.
   status_t GetNextFieldName(String & setFieldName) { if (HasData() == false) return B_ERROR; setFieldName = GetFieldName(); (*this)++; return B_NO_ERROR; }
   const String * GetNextFieldName() { if (HasData() == false) return NULL; const String * r = &GetFieldName(); (*this)++; return r; }
